...
describe("Client", () => {
  ...
  describe("#request", () => {
    describe("without credentials", () => {
      ...
      it("should handle error correctly", (done) => {
        client.request(
          { pathname: "/my-error-path", method: "POST" },
          (err, res) => {
      ...
      it("should handle OSS internal error correctly", (done) => {
        client.request(
          { pathname: "/my-internal-error-path", method: "POST" },
          (err, res) => {
            expect(err instanceof Error).to.be.true;
      ...
      describe("when the server is not reachable", () => {
        ...
        it("gives a useful error message", (done) => {
          client.request({ pathname: "/my-path", method: "POST" }, (err) => {
            const msg =
              "Cannot connect to OpenSearchServer at http://localhost:8080/my-path";
  ...
  describe("instances", () => {
    beforeEach(() => {
      nock("http://host1:9090").get("/x").reply(200, { host: 1 });

      nock("http://host2:9090").get("/x").reply(200, { host: 2 });
    });
